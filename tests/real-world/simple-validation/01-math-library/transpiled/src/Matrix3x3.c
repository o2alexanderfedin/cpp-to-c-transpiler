...
struct Matrix3x3 Matrix3x3__multiply__constclassMatrix3x3ref(struct Matrix3x3 * this, const struct Matrix3x3 * other) {
	...
	for (int row = 0; row < 3; row++) {
        for (int col = 0; col < 3; col++) {
                float sum = 0.F;
                for (int k = 0; k < 3; k++) {
                        sum = sum + this->data[row * 3 + k] * other.data[k * 3 + col];
                }
...
struct Vector3D Matrix3x3__multiply__constclassVector3Dref(struct Matrix3x3 * this, const struct Vector3D * vec) {
	return (class Vector3D){(class Vector3D){this->data[0] * vec.x + this->data[1] * vec.y + this->data[2] * vec.z, this->data[3] * vec.x + this->data[4] * vec.y + this->data[5] * vec.z, this->data[6] * vec.x + this->data[7] * vec.y + this->data[8] * vec.z}};
;
}
