...
#define ASSERT_NOT_EMPTY(str, msg) if ((str).empty()) { TEST_FAIL("", msg); return; }

// Helper: Parse C++ code and return CXXRecordDecl
CXXRecordDecl* parseClassDecl(const std::string& code, const std::string& className) {
...
void test_SimpleVirtualMethod() {
    ...
    ASSERT_CONTAINS(vtable, "struct Shape_vtable", "Should have Shape_vtable struct");
...
void test_VirtualMethodOverride() {
    ...
    ASSERT(analyzer.isPolymorphic(derivedRecord), "Derived should be polymorphic");

    // Generate vtable for derived class
    std::string vtable = generator.generateVtableStruct(derivedRecord);
    ASSERT_CONTAINS(vtable, "struct Derived_vtable", "Should have Derived_vtable");
...
void test_InheritedVirtualMethod() {
    ...
    ASSERT(analyzer.isPolymorphic(derivedRecord), "Derived should be polymorphic (inherited)");
...
void test_MixedVirtualNonVirtual() {
    ...
    auto virtualMethods = analyzer.getVirtualMethods(record);
    ASSERT(virtualMethods.size() == 2, "Should have 2 virtual methods (draw, area)");
...
void test_MultiLevelWithNewMethod() {
    ...
    ASSERT(virtualMethods.size() >= 3, "C should have at least 3 virtual methods");
...
void test_MultiLevelPartialOverride() {
    ...
    auto virtualMethods = analyzer.getVirtualMethods(recordC);
    ASSERT(virtualMethods.size() >= 2, "C should have at least 2 virtual methods");
...
void test_VirtualDestructor() {
    ...
    ASSERT(analyzer.isPolymorphic(record), "Base should be polymorphic (has virtual dtor)");

    std::string vtable = generator.generateVtableStruct(record);
    ASSERT_CONTAINS(vtable, "struct Base_vtable", "Should have Base_vtable");
    ASSERT_CONTAINS(vtable, "(*destructor)", "Should have destructor function pointer");
...
void test_PureVirtualMethod() {
    ...
    auto virtualMethods = analyzer.getVirtualMethods(record);
    ASSERT(virtualMethods.size() >= 1, "Should have at least 1 virtual method (draw)");
...
void test_MultipleAbstractMethods() {
    ...
    ASSERT(analyzer.isAbstractClass(abstractRecord), "AbstractBase should be abstract");
    ...
    ASSERT(!analyzer.isAbstractClass(concreteRecord), "Concrete should NOT be abstract");
...
void test_PolymorphicThroughPointer() {
    ...
    CXXMemberCallExpr* callExpr = parseMemberCallExpr(code);
    ASSERT(callExpr != nullptr, "Failed to parse member call");

    CXXRecordDecl* baseRecord = parseClassDecl(code, "Base");
    ASSERT(baseRecord != nullptr, "Failed to parse Base class");

    VirtualMethodAnalyzer analyzer(baseRecord->getASTContext());
    VirtualCallTranslator translator(baseRecord->getASTContext(), analyzer);
