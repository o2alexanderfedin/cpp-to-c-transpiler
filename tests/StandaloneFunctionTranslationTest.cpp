 ...
 * Tests translation of free functions, function overloading, main(), function calls
 *
...
void test_FunctionWithPointerReturn() {
    ...
    ASSERT(CFunc != nullptr, "C function not generated");
    ASSERT(CFunc->getReturnType()->isPointerType(), "Return type should be pointer");
...
void test_OverloadingDifferentParamCounts() {
    ...
    if (visitor.getCFunc("compute")) functionsFound++;
    if (visitor.getCFunc("compute_int")) functionsFound++;
    if (visitor.getCFunc("compute_int_int")) functionsFound++;
    if (visitor.getCFunc("compute_int_int_int")) functionsFound++;
