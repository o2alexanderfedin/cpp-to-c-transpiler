...
public:
   ...
   * 2. Map source path to target path via mapSourceToTarget()
   * 3. Return target path
   *
   * **Usage Pattern**:
   * ```cpp
   * PathMapper& mapper = getPathMapper();
   * std::string targetPath = mapper.getTargetPathFromDeclLocation(cppCtx, classDecl);
   * clang::TranslationUnitDecl* cTU = mapper.getOrCreateTU(targetPath);
   * ```
   *
   ...
   */
  std::string getTargetPathFromDeclLocation(const clang::ASTContext& cppASTContext, const clang::Decl* D);
