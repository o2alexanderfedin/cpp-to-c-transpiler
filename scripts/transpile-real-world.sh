...
transpile_project() {
    ...
    # Find all .cpp files only (excluding build directories)
    # Note: We only transpile .cpp files, not headers, as headers will be generated by the transpiler
    local files=()
    while IFS= read -r -d '' file; do
        files+=("$file")
    done < <(find "$project_dir" -name "*.cpp" ! -path "*/build/*" ! -path "*/transpiled/*" -print0)
