...
std::string PathMapper::getTargetPathFromDeclLocation(const clang::ASTContext& cppASTContext, const clang::Decl* D) {
  ...
  std::string targetPath = mapSourceToTarget(sourcePath);

  llvm::outs() << "[PathMapper::getTargetPathFromDeclLocation]\n";
  llvm::outs() << "  Decl:   " << (const void*)D << "\n";
  llvm::outs() << "  Source: " << sourcePath << "\n";
  llvm::outs() << "  Target: " << targetPath << "\n";
